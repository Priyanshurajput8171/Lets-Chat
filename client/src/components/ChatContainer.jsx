...
const ChatContainer = ({selectedUser,setSelectedUser}) => {
  ...
  useEffect(() =>{
    if(scrollEnd.current){
      scrollEnd.current.scrollInfoView({behavior : 'smooth'})
    }
  },[])
  return selectedUser ? (
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messagesDummyData.map((msg,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f5116f10f3cd28382ed02' && 'flex-row-reverse'}`}>
            {msg.image ? (
              <img src={msg.image} alt="" className='max-w-[230px] border border-gray-700 rounded-b-lg overflow-hidden mb-8'/>
            ):(
            ...
            <div className='text-center text-xs'>
              <img src={assets.senderId === '680f50e4f10f3cd28382ecf9' ? assets.avatar_icon : assets.profile_martin} alt="" className='w-7 rounded-full'/>
    ...
    <div className='absolute bottom-0 left-0 right-0 flex items-center gap-2 p-3'>
      <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
        <input type="text" placeholder='Send a message' className='flex-1 text-sm p-3 border-none rounded-lg outline-none text-white'/>
